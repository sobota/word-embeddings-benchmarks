...
def test_logreglearn():
    ...
    f_f1score, _ = learn_logit_reg(embedding=emb, cleaned_norms=cleaned, features=features, concepts=words, n_jobs=45,
                                   max_iter=1000,
                                   nb_hyper=2)
    ...
    generate_figure(f_f1score, 'TEST FIGURE', norms_path='../experiments/CSLB/norms.dat', fig_path=save_fig_path)
...
def test_learn_logits_on_syntetic_CSLB():
    ...
    f1, _ = learn_logit_reg(emb, features, words, cleaned, n_jobs=45, max_iter=1000, nb_hyper=2)
